...
class MatrixPreferenceDataModel(BaseDataModel):
    ...
    def preferences_from_user(self, user_id, order_by_id=True):
        ...
        un_nan_data_list = [(item_id, preference) for item_id, preference in data if not np.isnan(preference)]
        if order_by_id:
            return un_nan_data_list
        else:
            return sorted(un_nan_data_list, key=lambda item: - item[1])
    ...
    def preferences_for_item(self, item_id, order_by_id=True):
        ...
        un_nan_data = [(user_id, preference) for user_id, preference in data if not np.isnan(preference)]
        if order_by_id:
            return un_nan_data
        else:
            return sorted(un_nan_data, key=lambda user: - user[1])
